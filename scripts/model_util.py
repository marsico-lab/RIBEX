...
class newModel1(nn.Module):
    def __init__(self, vocab_size=26, Emb_dim = 1024):
        super().__init__()
        self.hidden_dim = 256
        self.batch_size = 256
    ...
    def forward(self, x):
        ...
        hn=hn.permute(1,0,2)        
        print("output shape1:"+str(output.shape))
        print("hidden shape1:"+str(hn.shape))
        output=output.reshape(output.shape[0],-1)
        hn=hn.reshape(output.shape[0],-1)

        print("output shape2:"+str(output.shape))
        print("hidden shape2:"+str(hn.shape))
